/** log */
#define LOG_TAG "OpenMaxActivity"
#define DGB 1
...
#include "log.h"
#include "com_example_openmax_OpenMaxActivity.h"
...
XAuint32 itemsLength /* input */) {
	...
	assert(dataCache <= (char *) pBufferData && (char *) pBufferData < &dataCache[BUFFER_SIZE * NB_BUFFERS]);
	...
	if (bytesRead > 0) {
		if ((bytesRead % MPEG2_TS_PACKET_SIZE) != 0) {
			ALOGD("Dropping last packet because it is not whole");
		}
		size_t packetsRead = bytesRead / MPEG2_TS_PACKET_SIZE;
		size_t bufferSize = packetsRead * MPEG2_TS_PACKET_SIZE;
		res = (*caller)->Enqueue(caller, NULL /*pBufferContext*/, pBufferData /*pData*/, bufferSize /*dataLength*/,
		...
		NULL /*pData*/, 0 /*dataLength*/, msgEos /*pMsg*/, sizeof(XAuint32) * 2 /*msgLength*/);
		assert(XA_RESULT_SUCCESS == res);
		reachedEof = JNI_TRUE;
	}
...
static void StreamChangeCallback(XAStreamInformationItf caller, XAuint32 eventId, XAuint32 streamIndex, void * pEventData,
		void * pContext) {
	ALOGD("StreamChangeCallback called for stream %u", streamIndex);
	...
	case XA_STREAMCBEVENT_PROPERTYCHANGE: {
		...
		case XA_DOMAINTYPE_VIDEO: {
			...
			assert(XA_RESULT_SUCCESS == res);
			ALOGD("Found video size %u x %u, codec ID=%u, frameRate=%u, bitRate=%u, duration=%u ms", videoInfo.width, videoInfo.height,
...
static jboolean enqueueInitialBuffers(jboolean discontinuity) {
	...
	size_t packetsRead = bytesRead / MPEG2_TS_PACKET_SIZE;
	ALOGD("Initially queueing %u packets", packetsRead);
	...
	for (i = 0; i < NB_BUFFERS && packetsRead > 0; i++) {
		...
		if (discontinuity) {
			...
			//   plus the size if itemSize, both XAuint32
			res = (*playerBQItf)->Enqueue(playerBQItf, NULL /*pBufferContext*/, dataCache + i * BUFFER_SIZE, bufferSize, items /*pMsg*/,
 ...
 * Signature: ()V
 */
JNIEXPORT void JNICALL Java_com_example_openmax_OpenMaxActivity_createEngine(JNIEnv *, jclass) {
	XAresult res;
 ...
 */
JNIEXPORT jboolean JNICALL Java_com_example_openmax_OpenMaxActivity_createStreamingMediaPlayer(JNIEnv * env, jclass, jstring filename) {
 ...
 * Signature: (Z)V
 */
JNIEXPORT void JNICALL Java_com_example_openmax_OpenMaxActivity_setPlayingStreamingMediaPlayer(JNIEnv *, jclass, jboolean isPlaying) {
	XAresult res;
	...
	if (NULL != playerPlayItf) {
		...
		res = (*playerPlayItf)->SetPlayState(playerPlayItf, isPlaying ? XA_PLAYSTATE_PLAYING : XA_PLAYSTATE_PAUSED);
		assert(XA_RESULT_SUCCESS == res);
 ...
 * Signature: (I)V
 */
JNIEXPORT void JNICALL Java_com_example_openmax_OpenMaxActivity_setSurface(JNIEnv *env, jclass, jobject surface) {
 ...
 * Signature: ()V
 */
JNIEXPORT void JNICALL Java_com_example_openmax_OpenMaxActivity_rewindStreamingMediaPlayer(JNIEnv *, jclass) {
	XAresult res;
 ...
 * Signature: ()V
 */
JNIEXPORT void JNICALL Java_com_example_openmax_OpenMaxActivity_shutdown(JNIEnv *, jclass) {
